...
class Gun(IWeapon):
    """총  클라스 """
    def __init__(self, damage, num_round):
        self.damage = damage
        self.num_round = num_round

    def use_on(self, other_characer):
        if self.num_round > 0:
            other_characer.get_damage(self.damage)
            self.num_round -= 1
        else: 
            print("총알이 없어서 공격할 수 없습니다.")
...
game_character1 = GameCharacter('Bananana', 100, bad_sword)
game_character2 = GameCharacter('Grape', 1000, gun)

game_character1.attack(game_character2)
game_character1.attack(game_character2)
game_character1.attack(game_character2)
game_character2.attack(game_character1)

print(game_character1)
print(game_character2)
